...
#define NUM_FBLOCKS 64          // Number of file blocks in the cache.
                                // Note: must be power of 2, may be partially hardcoded
...
// A file block struct, which is simply a block of data BLOCK_SECTOR_SIZE bytes.
struct fblock
{
...
// Mark the cache block as accessed.
static inline void fblock_set_accessed(uint32_t *status)
{
    *status |= FBLOCK_A;
}

// Mark the cache block as not used.
...
// Mark the cache block as not accessed.
static inline void fblock_set_not_accessed(uint32_t *status)
{
    *status &= ~FBLOCK_A;
}

// Returns true if passed status indicates file block is used, and false
// otherwise.
...
// Returns true if passed status indicates file block is dirty, and false
// otherwise.
...
// Returns true if passed status indicates file block is accessed, and false
// otherwise.
